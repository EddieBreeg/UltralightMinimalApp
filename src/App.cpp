...
#define METHOD_CALLBACK(funcName)                                              \
	[](JSContextRef ctx, JSObjectRef f, JSObjectRef thisObj, size_t n,         \
...
TestApp::TestApp(const char *title, uint32_t width, uint32_t height) {
	_app = App::Create();
	_win = Window::Create(_app->main_monitor(), width, height, false,
...
void TestApp::OnDOMReady(ultralight::View *caller, uint64_t frame_id,
	...
	auto func = JSObjectMakeFunctionWithCallback(ctx, (JSStringRef)funcName,
												 METHOD_CALLBACK(OnButton));
	JSObjectSetProperty(ctx, JSContextGetGlobalObject(ctx), funcName, func, 0,
						0);
}

JSValueRef TestApp::OnButton(JSContextRef ctx, JSObjectRef, JSObjectRef, size_t,
